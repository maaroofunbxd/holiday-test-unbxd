...
#   HOST=http://host.com ./accessloadtestcluster.sh @loadtestcluster-commands.sh  # Read commands from file
#   REGION=use-1d HOST=http://host.com ./accessloadtestcluster.sh @loadtestcluster-commands.sh  # Specify region and host
...
#HOST=$(ssh ec2-user@usejump.unbxd.io "bash -s" -- < ./accesscluster.sh "./get-service-host.sh reranker search")
...
#HOST=http://$(./accesscluster.sh use-1d "./get-service-host.sh ner-demo search" 2>&1 | grep -oE '[a-z0-9-]+\.[a-z0-9-]+\.elb\.amazonaws\.com' | head -1) REGION=use-1d ./accessloadtestcluster.sh @loadtestcluster-commands.sh
