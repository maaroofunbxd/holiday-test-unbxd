...
SOURCE_IMAGE=$(kubectl get deployment $deploymentname -n$namespace -o jsonpath="{.spec.template.spec.containers[?(@.name=='goreranker')].image}")
echo "SOURCE_IMAGE: $SOURCE_IMAGE"
CURRENT_IMAGE=$(kubectl get deployment $deploymentname-demo -n$namespace -o jsonpath="{.spec.template.spec.containers[?(@.name=='goreranker')].image}")
echo "CURRENT_IMAGE: $CURRENT_IMAGE"
kubectl set image deployment/$deploymentname-demo -n$namespace goreranker=$SOURCE_IMAGE


POLICY=$(kubectl get deploy $deploymentname -n$namespace -o jsonpath='{.spec.template.spec.containers[?(@.name=="goreranker")].imagePullPolicy}')
kubectl patch deploy $deploymentname-demo -n$namespace -p "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"goreranker\",\"imagePullPolicy\":\"$POLICY\"}]}}}}"
kubectl get deploy $deploymentname-demo -n$namespace -o jsonpath='{.spec.template.spec.containers[?(@.name=="goreranker")].imagePullPolicy}'
...
CONTAINER=$(kubectl get deploy $deploymentname -n$namespace -o json | jq -c '.spec.template.spec.containers[] | select(.name=="goreranker") | {name,resources,livenessProbe,readinessProbe}')

# 2️⃣ Print existing target container values before patching
echo "---- Current target values ----"
kubectl get deploy $deploymentname-demo -n$namespace -o json | jq '.spec.template.spec.containers[] | select(.name=="goreranker") | {name,resources,livenessProbe,readinessProbe}'
...
kubectl get deploy $deploymentname-demo -o json | jq '.spec.template.spec.containers[] | select(.name=="goreranker") | {name,resources,livenessProbe,readinessProbe}'

kubectl get deploy $deploymentname-demo -n$namespace -o jsonpath='{.spec.template.spec.containers[?(@.name=="goreranker")].livenessProbe}'
kubectl get deploy $deploymentname-demo -n$namespace -o jsonpath='{.spec.template.spec.containers[?(@.name=="goreranker")].readinessProbe}'
kubectl get deploy $deploymentname-demo -n$namespace -o jsonpath='{.spec.template.spec.containers[?(@.name=="goreranker")].resources}'



kubectl set env deployment/$deploymentname -n$namespace --containers="goreranker" LOG_LEVEL=error
kubectl set env deployment/reranker -n$namespace --containers="$containername" LOG_LEVEL=ERROR
kubectl annotate deploy $deploymentname -n search \
...
#apse1
kubectl scale deploy $deploymentname-demo -n$namespace --replicas=$replicas
