...
#   ./accesscluster.sh use-1d @podcluster-commands.sh      # Read commands from file with region
...
if [[ "${2}" == @* ]]; then
  ...
  CMD_FILE="${2:1}"
  if [ -f "$CMD_FILE" ]; then
    # Read file content and execute with error stopping
    CMD="set -e; $(cat "$CMD_FILE")"
...
if [ "$REGION" = "use-1d" ]; then
  ...
  SSH_TYPE="standard"
elif [ "$REGION" = "use-1dprod" ]; then
  SSH_HOST="10.0.40.71"
  SSH_USER="prod02-us-east-1-eks"
  SSH_TYPE="standard"
elif [ "$REGION" = "ap-southeast-1prod" ]; then
  SSH_HOST="10.204.19.93"
  SSH_USER="prod01-ap-southeast-1-eks"
  SSH_TYPE="standard"
